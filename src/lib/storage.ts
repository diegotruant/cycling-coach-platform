...
export interface AthleteDocument {
    ...
    uploadedAt: string;
    metadata?: any;
    filename?: string; // Usually stored in metadata but helpful here
}
...
export async function getAthleteDocuments(athleteId: string): Promise<AthleteDocument[]> {
    const { rows } = await pool.query(`
        SELECT * FROM athlete_documents WHERE athlete_id = $1 ORDER BY uploaded_at DESC
    `, [athleteId]);
    ...
    return rows.map(row => ({
        ...
        filename: row.metadata?.filename || row.storage_path?.split('/').pop() || 'document'
    }));
}

export async function saveDocument(athleteId: string, doc: Partial<AthleteDocument>): Promise<void> {
    // If updating existing, we might have an ID. If new, ID is generated by default or passed.
    if (doc.id) {
        await pool.query(`
            UPDATE athlete_documents SET
            ...
            WHERE id = $1 AND athlete_id = $8
        `, [
            doc.id, doc.type, doc.fileUrl, doc.storagePath, doc.status, doc.expirationDate, doc.metadata, athleteId
        ]);
    } else {
        await pool.query(`
            INSERT INTO athlete_documents (athlete_id, type, file_url, storage_path, status, expiration_date, metadata)
            VALUES ($1, $2, $3, $4, $5, $6, $7)
        `, [
            athleteId, doc.type, doc.fileUrl, doc.storagePath, doc.status || 'PENDING', doc.expirationDate, doc.metadata || {}
        ]);
    }
}

export async function deleteDocument(docId: string, athleteId: string): Promise<void> {
    await pool.query('DELETE FROM athlete_documents WHERE id = $1 AND athlete_id = $2', [docId, athleteId]);
}
